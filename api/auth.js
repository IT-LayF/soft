...
export default async function handler(req,res) {
  ...
  try {
    ...
    if (action==='register') {
      ...
      const exists=await sql`select 1 from users where lower(login)=lower(${login}) or lower(nickname)=lower(${nickname})`;
      if(exists.length) return json(res,409,{message:'Логин или Minecraft-ник уже занят'});
    ...
    if(action==='login') {
      ...
      let rows=await sql`select * from users where lower(login)=lower(${requestedLogin}) or lower(nickname)=lower(${requestedLogin}) limit 1`;
      ...
      const u=rows[0];
      if(!u||u.blocked||!await verifyPassword(requestedPassword,u.password_hash)) return json(res,401,{message:'Неверный логин, Minecraft-ник или пароль'});
